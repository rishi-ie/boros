...
def loop_advance_stage(params: dict, kernel=None) -> dict:
    ...
    # Also update loop-orchestrator state
    lo_state = os.path.join(boros_dir, "skills", "loop-orchestrator", "state", "loop_state.json")
    if os.path.exists(os.path.dirname(lo_state)):
