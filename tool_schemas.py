...
TOOL_SCHEMAS = {
    ...
    # ── Memory ──
    "core_memory_append": _s("core_memory_append", "Append text to a core memory block (Frontal Lobe). Use this immediately when the Director states a preference, rule, or architectural fact.", {"block": {"type": "string", "enum": ["persona_status", "director_dossier"]}, "content": {"type": "string"}}, ["block", "content"]),
    "core_memory_replace": _s("core_memory_replace", "Completely replace a core memory block (Frontal Lobe).", {"block": {"type": "string", "enum": ["persona_status", "director_dossier"]}, "content": {"type": "string"}}, ["block", "content"]),
    "memory_store": _s("memory_store", "Store a memory node in the RLM system. Use for episodes (what happened), patterns (recurring behaviors), procedures (how-to), causal facts (skill cause-effect), or evolution records. Prefer this over memory_commit_archival.", {"type": {"type": "string", "enum": ["episode", "pattern", "procedure", "causal", "evolution", "lesson", "observation"]}, "title": {"type": "string", "description": "Short human-readable title"}, "content": {"type": "string", "description": "Full text. Use Context:/Action:/Outcome: format for episodes."}, "tags": {"type": "array", "items": {"type": "string"}}, "links": {"type": "array", "items": {"type": "string"}, "description": "IDs of related memory nodes to link (creates bidirectional backlinks)"}, "subject": {"type": "string", "description": "Causal only: entity (skill name, category)"}, "predicate": {"type": "string", "description": "Causal only: relationship type (has_score, caused_delta_in, was_modified, achieved_milestone)"}, "object": {"type": "string", "description": "Causal only: value or entity"}, "cycle": {"type": "integer", "description": "Causal only: cycle number"}, "metadata": {"type": "object"}}, ["type"]),
    "memory_retrieve": _s("memory_retrieve", "Retrieve relevant memories using RLM (Retrieval Loop Memory) graph traversal. Returns a DRG brief with section-organized context plus a flat results list. Prefer this over memory_search_sql.", {"query": {"type": "string", "description": "What you're looking for"}, "intent": {"type": "string", "enum": ["orient", "evolve", "reflect", "work", "general", "causal_query", "pattern_query", "procedure_query"], "description": "Query intent — determines which section types are prioritized"}, "tags": {"type": "array", "items": {"type": "string"}}, "seed_ids": {"type": "array", "items": {"type": "string"}, "description": "Start traversal from specific memory node IDs"}, "token_budget": {"type": "integer", "description": "Max tokens to load (default 4000)"}}, ["query"]),
    "memory_search_sql": _s("memory_search_sql", "[DEPRECATED — use memory_retrieve] Search memory files using a keyword query.", {"query": {"type": "string"}}, ["query"]),
    "memory_commit_archival": _s("memory_commit_archival", "[DEPRECATED — use memory_store] Commit an entry to long-term archival memory.", {"entry_type": {"type": "string", "enum": ["lesson", "observation", "fact"]}, "content": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}}, ["entry_type", "content"]),
    "memory_kg_write": _s("memory_kg_write", "[DEPRECATED — use memory_store(type='causal')] Record a temporal fact about a skill or category.", {"subject": {"type": "string"}, "predicate": {"type": "string"}, "object": {"type": "string"}, "cycle": {"type": "integer"}, "metadata": {"type": "object"}}, ["subject", "predicate", "object"]),
    "memory_kg_query": _s("memory_kg_query", "[DEPRECATED — use memory_retrieve] Query facts about a skill or category.", {"subject": {"type": "string"}, "predicate": {"type": "string"}, "include_history": {"type": "boolean"}, "as_of": {"type": "string"}}, ["subject"]),
